...
class GradleTask(
	...
	private fun singleProjectTask(
		...
		if (!isEnabled(gradleProjectContext)) {
			...
			return TaskResult.success("Gradle task: $id not enabled on gradle project: $gradleProjectContext. Skipping...")
		}
		println("Executing Gradle task: $id with args: ${args.joinToString(", ")} over gradle project: $gradleProjectContext")
		val gradleProjectDir = Path(projectContext.dir.toString(), gradleProjectContext.path).toAbsolutePath()
		val gradleCommand = gradleProjectDir.resolve(gradleProjectContext.gradlePath)
		val processBuilder = ProcessBuilder(gradleCommand.toString(), command, *args.toTypedArray())
			.directory(gradleProjectDir.toFile())
			.inheritIO()
		val process = processBuilder.start()
		val exitCode = process.waitFor()
		return if (exitCode != 0) {
			TaskResult.failure("Gradle task: $id over gradle project: $gradleProjectContext failed with exit code $exitCode")
		} else {
			TaskResult.success("Gradle task: $id over gradle project: $gradleProjectContext completed successfully")
		}
	}
